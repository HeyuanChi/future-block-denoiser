...
@dataclass
class FutureLatentInitializerConfig:
    latent_dim: int = 256
    prefix_len: int = 64
    future_len: int = 16
    initializer_layers: int = 2
    initializer_heads: int = 8
    initializer_ffn_dim: int = 1024
    initializer_dropout: float = 0.1

    @classmethod
    def from_dict(cls, config: dict[str, Any]) -> "FutureLatentInitializerConfig":
...
class FutureLatentInitializer(nn.Module):
    """
    Predicts a coarse future latent block from prefix conditioning states.
    ...
    Inputs:
        ...
        future_mask: [B, F]
    Output:
        coarse_future_latent: [B, F, D]
    """

    def __init__(self, config: FutureLatentInitializerConfig) -> None:
        super().__init__()
        self.config = config

        self.future_queries = nn.Parameter(torch.randn(config.future_len, config.latent_dim) * 0.02)
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            nhead=config.initializer_heads,
            dim_feedforward=config.initializer_ffn_dim,
            dropout=config.initializer_dropout,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        self.transformer = nn.TransformerEncoder(
            ...
            num_layers=config.initializer_layers,
        )
    ...
    def forward(
        ...
        future_states = self.future_queries.unsqueeze(0).expand(batch_size, future_len, -1)
        token_states = torch.cat([prefix_states, future_states], dim=1)

        position_ids = torch.arange(prefix_len + future_len, device=prefix_states.device)
        ...
        segment_ids = torch.cat(
            [
                ...
                torch.ones(batch_size, future_len, device=prefix_states.device, dtype=torch.long),
            ],
            dim=1,
        )
